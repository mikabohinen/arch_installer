...
# Uncomment to set the keymap you want. Replace "no" by your country code
...
if [ ! -d "$DOTFILES" ]; then
    ...
    git clone https://github.com/mikabohinen/dotfiles.git \
    "$DOTFILES" >/dev/null
fi
...
# Ask to switch caps and escape
dialog --title "Switch caps and esc" --yesno \
"Do you want to switch the caps and esc key?" \
10 60 \
&& curl https://raw.githubusercontent.com/mikabohinen\
/arch_installer/master/switch_caps_esc.sh > /tmp/switch_caps_esc.sh \
&& bash /tmp/switch_caps_esc.sh
